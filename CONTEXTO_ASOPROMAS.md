...
- **Estilos:** Tailwind CSS (colores corporativos `emerald`, `amber`, `slate`), íconos `lucide-react`
...
2. **UI:** cards con `rounded-2xl`/`rounded-3xl`, `shadow-sm`, `transition-colors`, `hover:-translate-y-1`.
3. **Carga/errores:** `animate-spin` en `emerald`, alertas claras, modales con `backdrop-blur-sm`.
...
```
Actualiza CONTEXTO_ASOPROMAS.md con el siguiente avance.
